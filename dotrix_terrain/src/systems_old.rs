...
pub fn spawn(
    ...
    println!(
        "Spawn terrain in {}ms",
        (std::time::Instant::now() - begin).as_millis()
    );
}
...
fn queue_tiles_to_spawn(
    ...
    let dx = (position.x as f32 - viewer.position[0]).abs();
    let dz = (position.z as f32 - viewer.position[1]).abs();
    let distance_sq = dx * dx + dz * dz;
    let lod_distance = (2 * half_tile_size) as f32;
    let x = position.x;
    let z = position.z;

    if dx > lod_distance || dz > lod_distance || lod == 0 {
        ...
        tile.visible = true;
        println!("QUEUE < LOD{} @ {}:{}", lod, position.x, position.z);
    } else {
...
fn queue_tiles_to_spawn_circle(
    ctx: &mut Spawner,
    viewer: &Viewer,
    half_tile_size: i32,
    lod: usize,
...
pub fn render(
    ...
    for (tile, material, pipeline) in query {
        ...
        if !pipeline.ready() {
            println!("Init terrain pipeline {} for LOD{}", count, tile.lod);
